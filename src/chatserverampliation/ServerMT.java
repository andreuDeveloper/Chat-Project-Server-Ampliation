...
public class ServerMT extends Thread {
    ...
    /**
     * Method that check if the in is from a server, client or other font and do
     * the pertinent thinhs
     *
     * @param in
     */
    private void checkIfClientOrServer(Socket socket) {
        ...
        try {
            ...
            switch (line.substring(0, line.lastIndexOf("$") + 1)) {
                case "$SERVER$": //Server //recoger parametro port
                    int port = Integer.parseInt(line.substring(line.lastIndexOf("$") + 1, line.length()));
                    System.out.println("PORT: " + port);
                    
                    //if no existe ese server && haySitio para server lo añadimos

                    System.out.println("Servidor received");
//                    
                    if (sp.canAddServer(host, port)) {
                        this.sp.addServer(this.socket, port);
                    }
